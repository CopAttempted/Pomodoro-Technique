...
root.title("Pomodoro Technique App")

tomate = PhotoImage(file="./tomato.png")
root.iconphoto(False, tomate)

window_width = 500
window_height = 500
root.geometry(
    f"{window_width}x{window_height}+{(root.winfo_screenwidth() - window_width) // 2}+{(root.winfo_screenheight() - window_height) // 2}"
)

upper_text = Label(
    text="Timer", font=("Arial", 40, "bold"), foreground=GREEN, background=YELLOW
)
upper_text.grid(column=1, row=0, pady=(25, 0))

timer = Canvas(root, highlightthickness=0, background=YELLOW, width=200, height=220)
timer.create_image(100, 100, image=tomate)
timer_text = timer.create_text(
    100, 120, text=time, font=("Arial", 20, "bold"), fill=GREEN
)
